...
def sity(message):
    global Sity
    Sity = message.text
    msg1 = bot.send_message(message.chat.id, f'You have chosen <b>an apartment in Russia</b> \nin the sity: <b>{Sity}</b>\n\nEnter your phone number', parse_mode='html')
...
def name(message):
    ...
    i_next = types.InlineKeyboardButton(text='Skip', callback_data='nextEn5')
    nMarkUp.add(i_next)
...
def email(message):
    ...
    item_res = types.InlineKeyboardButton(text='At the beginning', callback_data='restEn')
